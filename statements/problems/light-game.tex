...
\end{itemize}
Each light exists in one of two states: \textit{off} or \textit{on}.
...
\newline both its neighbors become on
\item If you press a light where both its neighbors are on,
\newline its state becomes flipped \textit{and} both its neighbors become off.
\item If the pressed light has the same state as its left neighbor but not its right,
\newline the states of its two neighbors \textit{both} become flipped.
\item If the pressed light has the same state as its right neighbor but not its left,
\newline the state of the pressed light changes to become the same as its \textit{left} neighbor.
\end{itemize}
...
\begin{center}
    \includegraphics[width=0.85\linewidth]{img/light-press-b.png}
