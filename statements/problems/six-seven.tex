...
For another example, \texttt{(7+7) - (6+7 - 7)/6} equals \texttt{10}, evaluated as follows:
\begin{align*}
    \texttt{(7+7) - (6+7 - 7)/6}
...
\begin{itemize}
    ...
    \item For \texttt{+} and \texttt{*}, leading zeros are removed at each intermediate step, e.g. \texttt{0 + 0} and \texttt{0 * 5} both evaluate to \texttt{0}
